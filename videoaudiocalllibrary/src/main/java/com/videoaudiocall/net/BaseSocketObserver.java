...
public abstract class BaseSocketObserver<T> extends DisposableObserver<T> {
    ...
    public BaseSocketObserver(IView view) {
        this.view = view;
    }
    ...
    public void onNext(T bean) {
        try {
            BaseSocketResult model = (BaseSocketResult) bean;
            if (model.getCode() == 0) {
                onSuccess(bean);
            } else {
                onError(model.getMsg() == null ? PubUtil.ERROR_NOTICE : model.getMsg());
            }
